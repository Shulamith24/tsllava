...
实验 D 的核心组件：用小的从头训练 Transformer 替代预训练 LLM
...
class SmallTransformerAggregator(nn.Module):
    ...
    用于实验 D：验证是否需要预训练 LLM，还是小 Transformer 就足够
    ...
    def forward(
        ...
        # PyTorch Transformer: True 表示忽略，False 表示保留
...
def create_small_transformer_aggregator(
    ...
    **kwargs
    ...
    Args:
        config_name: 配置名称
            - "small": 2层, 512维 (~6M 参数)
            - "medium": 3层, 768维 (~14M 参数) [推荐]
            - "large": 4层, 1024维 (~32M 参数)
        **kwargs: 覆盖默认配置
    """
    configs = {
        "small": {
    ...
    return SmallTransformerAggregator(**config)
