...
class NewTSVisionEncoder(nn.Module):
    ...
    def ts2image(
        ...
        *,
        ...
        if stride == 1.0:
            pad_right = (patch_size - time_length % patch_size) % patch_size
            if pad_right > 0:
                x = F.pad(x, (0, pad_right), mode="replicate")
            x_2d = einops.rearrange(x, "b d (n p) -> (b d) 1 n p", p=patch_size)
        else:
            stride_length = max(1, int(round(patch_size * stride)))
            if time_length < patch_size:
                pad_right = patch_size - time_length
            else:
                pad_right = (stride_length - (time_length - patch_size) % stride_length) % stride_length
            if pad_right > 0:
                x = F.pad(x, (0, pad_right), mode="replicate")
            x_2d = x.unfold(dimension=2, size=patch_size, step=stride_length)
