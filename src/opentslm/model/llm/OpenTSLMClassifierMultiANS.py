...
"""
OpenTSLMClassifierMultiANS: LLM-based Time Series Classifier with Multiple ANS Tokens

实验 C: 验证多个 query tokens 是否能更好地聚合时序信息

核心设计：
- P=0（无 prefix）
- M 个可学习的 [ANS] tokens
- 分类方法：Token-wise LayerNorm + 共享 Linear + 平均 logits

输入序列格式：
    [TS tokens] + [ANS_1, ANS_2, ..., ANS_M]
"""
...
class OpenTSLMClassifierMultiANS(TimeSeriesLLM):
    """
    LLM-based Time Series Classifier using Multiple ANS Tokens
    
    核心组件：
    - encoder: 时间序列编码器
    - projector: 将编码器输出投影到 LLM hidden space
    - llm: Llama 预训练模型（支持 LoRA 微调）
    - ans_tokens: M 个可学习的 ANS query vectors
    - token_norm: Token-wise LayerNorm
    - classifier_head: 共享的分类头 (hidden_size -> num_classes)
    """

    def __init__(
        self,
        num_classes: int,
        num_ans_tokens: int = 4,
        ...
        Args:
            num_classes: 分类类别数
            num_ans_tokens: ANS tokens 数量 (M)
            llm_id: LLM 模型 ID
            device: 设备
        ...
        self.num_ans_tokens = num_ans_tokens

        # 1) tokenizer (仅用于 EOS token)
        ...
        # 4) M 个 ANS tokens
        self.ans_tokens = nn.Parameter(
            torch.randn(num_ans_tokens, self.llm.config.hidden_size, 
                       device=device, dtype=torch.bfloat16) * 0.02
        )

        # 5) Token-wise LayerNorm
    ...
    def pad_and_apply_batch(
        ...
        Returns:
            inputs_embeds: [B, L, H]
            attention_mask: [B, L]
            ans_start_positions: [B] (第一个 ANS token 的位置)
        """
        device = self.device
        ...
        # 2) 为每个样本构建序列：[TS tokens] + [ANS tokens]
        all_seq_embeds = []
        ts_offset = 0
        ...
        for i, n_ts in enumerate(ts_counts):
            ...
            # 添加 M 个 ANS tokens
            seq_parts.append(self.ans_tokens)  # [M, H]
        ...
        # 4) 计算 ans_start_positions (第一个 ANS token 的位置)
        ans_start_positions = torch.tensor(
            [seq.shape[0] - self.num_ans_tokens for seq in all_seq_embeds],
            device=device, dtype=torch.long
        )  # [B]

        return inputs_embeds, attention_mask, ans_start_positions
    ...
    def compute_loss(self, batch: List[Dict[str, any]]) -> torch.Tensor:
        ...
        分类方法：
        1. 提取 M 个 ANS tokens 的 hidden states
        ...
        # 获取输入 embeddings
        inputs_embeds, attention_mask, ans_start_positions = self.pad_and_apply_batch(batch)
        ...
        # 提取 ANS tokens 的 hidden states
        last_hidden_states = outputs.hidden_states[-1]  # [B, L, H]
        B = last_hidden_states.size(0)
        M = self.num_ans_tokens
        
        # 提取每个样本的 M 个 ANS tokens
        ans_states = []
        for i in range(B):
            start = ans_start_positions[i].item()
            ans_states.append(last_hidden_states[i, start:start+M, :])  # [M, H]
        
        ans_states = torch.stack(ans_states, dim=0)  # [B, M, H]

        # Token-wise LayerNorm
        ans_states = self.token_norm(ans_states)  # [B, M, H]

        # 分类头（每个 token 独立分类）
        logits = self.classifier_head(ans_states)  # [B, M, K]

        # 平均 logits
        logits = logits.mean(dim=1)  # [B, K]
    ...
    def predict(self, batch: List[Dict[str, any]]) -> torch.Tensor:
        ...
        for i in range(B):
            start = ans_start_positions[i].item()
            ans_states.append(last_hidden_states[i, start:start+M, :])
        
        ans_states = torch.stack(ans_states, dim=0)  # [B, M, H]

        # Token-wise LayerNorm
        ans_states = self.token_norm(ans_states)
