...
from opentslm.time_series_datasets.mitbih.mitbih_loader import (
    MITBIH_AAMI_LABEL_ORDER,
    MITBIH_WINDOW_SIZE,
    load_mitbih_arrhythmia_splits,
)


class MITBIHClassificationDataset(LazyQADataset):
    _cache: ClassVar[dict[tuple[str, str, int], dict[str, Any]]] = {}
    ...
    def __init__(
        ...
        EOS_TOKEN: str,
        dataset_name: str = "MITBIHArrhythmia",
        ...
        self._instance_window_size = int(window_size)
    ...
    def _get_pre_prompt(self, row) -> str:
        ...
        if self._instance_label_interface == "semantic":
            ...
            return (
                f"Classify the ECG heartbeat into one of the following diagnostic labels:\n"
                f"{options}\n\n"
        ...
        return (
            f"Classify the ECG heartbeat into one of {num_classes} AAMI arrhythmia classes.\n"
    ...
    def _get_text_time_series_prompt_list(self, row) -> List[TextTimeSeriesPrompt]:
        ...
        if std > 1e-6:
            ...
            tensor = tensor - mean
        prompt_text = (
            f"This is a univariate ECG heartbeat segment centered at the R-peak with {tensor.numel()} points:"
        )
    ...
    def _format_sample(self, row):
        ...
        sample["beat_offset"] = int(row["beat_offset"])
        sample["sample_index"] = int(row["sample_index"])
        sample["lead_name"] = row["lead_name"]
        return sample
