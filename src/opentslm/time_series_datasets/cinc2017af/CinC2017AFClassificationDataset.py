...
from opentslm.time_series_datasets.cinc2017af.cinc2017af_loader import (
    CINC2017AF_LABEL_ORDER,
    CINC2017AF_TARGET_LENGTH,
    load_cinc2017af_splits,
)


class CinC2017AFClassificationDataset(LazyQADataset):
    ...
    def _load_splits(self) -> Tuple[list[dict[str, Any]], list[dict[str, Any]], list[dict[str, Any]]]:
        ...
        if payload is None:
            train_rows, val_rows, test_rows = load_cinc2017af_splits(
            ...
            ordered_labels, label_to_token, token_to_label, class_tokens = build_label_token_mapping(
                CINC2017AF_LABEL_ORDER,
                ordered_labels=CINC2017AF_LABEL_ORDER,
            )
    ...
    def _get_pre_prompt(self, row) -> str:
        ...
        if self._instance_label_interface == "semantic":
            ...
            return (
                f"Classify the single-lead ECG recording into one of the following rhythm labels:\n"
                f"{options}\n\n"
        ...
        return (
            f"Classify the single-lead ECG recording into one of {num_classes} rhythm classes.\n"
    ...
    def _get_text_time_series_prompt_list(self, row) -> List[TextTimeSeriesPrompt]:
        ...
        if std > 1e-6:
            ...
            tensor = tensor - mean
        prompt_text = f"This is a univariate single-lead ECG recording with {tensor.numel()} points:"
