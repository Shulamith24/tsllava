...
def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="M2 no-LLM ablation: dual-branch encoder features plus a linear classifier."
    )
    ...
    parser.add_argument(
        ...
        default=True,
        help="Freeze the loaded dual-branch encoder and train only the linear head.",
    )
...
def resolve_encoder_config_from_checkpoint(
    ...
    if args.vision_2d_mode_explicit or args.vit_stride_explicit:
...
class DualBranchLinearClassifier(nn.Module):
    def __init__(
        self,
        *,
        ...
        self.classifier = nn.Linear(self.branch_feature_dim * 2, num_classes)
...
def build_encoder_linear_model(
    *,
    ...
    num_classes: int,
    device: torch.device,
) -> Tuple[DualBranchLinearClassifier, Dict[str, Any]]:
    ...
    model = DualBranchLinearClassifier(
        encoder=encoder,
...
def build_optimizer(model: DualBranchLinearClassifier, args: argparse.Namespace, hparams: ResolvedHParams) -> AdamW:
    ...
    if not param_groups:
        raise RuntimeError("No trainable parameters found for the no-LLM linear ablation.")
...
def train_model(
    ...
    *,
    ...
    if args.resume and checkpoint_path.exists():
        ...
        print(f"Resume no-LLM linear training from epoch {start_epoch - 1}/{hparams.epochs}")
    ...
    for epoch in range(start_epoch, hparams.epochs + 1):
        ...
        support_metrics = evaluate(
            ...
            support_eval_loader,
            device=device,
            num_local_classes=model.classifier.out_features,
        )
...
def run_single_experiment(
    *,
    ...
    if completed_run_exists:
        print(f"[m2_no_llm_linear shot={shot_name} run={run_id}] reuse completed run: {run_metrics_path}")
    ...
    print("-" * 80)
    print(
        f"[m2_no_llm_linear shot={shot_name} run={run_id}] "
    ...
    run_metrics = {
        ...
        "linear_head_input_dim": model.branch_feature_dim * 2,
...
def main(argv: Optional[Sequence[str]] = None) -> None:
    ...
    print("=" * 80)
    print("M2 No-LLM Linear: Few-shot Univariate Classification")
    print("=" * 80)
