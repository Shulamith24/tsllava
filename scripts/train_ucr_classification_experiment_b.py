...
"""
实验 B: 可学习 Prefix Tokens

验证文本 prompt 是否作为可学习控制符而非语义。

核心改动：
- 使用 OpenTSLMClassifierLearnablePrefix（可学习 prefix tokens）
...
def parse_args():
    parser = argparse.ArgumentParser(description="实验 B: 可学习 Prefix Tokens")
    ...
    # 模型相关
    parser.add_argument("--num_prefix_tokens", type=int, default=8, help="可学习 prefix tokens 数量（支持 0）")
    ...
    parser.add_argument("--lr_prefix", type=float, default=5e-4, help="Prefix tokens 学习率")
    parser.add_argument("--lr_classifier", type=float, default=1e-4, help="分类头学习率")
    ...
    # 保存相关
    parser.add_argument("--save_dir", type=str, default="results/experiment_b", help="结果保存目录")
...
def save_checkpoint(
    ...
    checkpoint = {
        ...
        "num_prefix_tokens": underlying_model.num_prefix_tokens,
    ...
    # 保存 prefix_tokens (如果存在)
    if underlying_model.prefix_tokens is not None:
        checkpoint["prefix_tokens"] = underlying_model.prefix_tokens.data.cpu()
...
def main():
    ...
    if rank == 0:
        print("=" * 60)
        print("实验 B: 可学习 Prefix Tokens")
        print("=" * 60)
        print(f"时间: {datetime.datetime.now()}")
        print(f"数据集: {args.dataset}")
        print(f"编码器: {args.encoder_type}")
        print(f"Prefix Tokens 数量: {args.num_prefix_tokens}")
    ...
    # 创建保存目录
    save_dir = os.path.join(args.save_dir, args.dataset, f"prefix_{args.num_prefix_tokens}")
    if rank == 0:
    ...
    # 添加 prefix_tokens (如果存在)
    if underlying_model.prefix_tokens is not None:
        param_groups.append({"params": [underlying_model.prefix_tokens], "lr": args.lr_prefix})
    
    param_groups.append({"params": underlying_model.classifier_head.parameters(), "lr": args.lr_classifier})
    param_groups.append({"params": [underlying_model.ans_token], "lr": args.lr_classifier})
    ...
    try:
        ...
        if rank == 0:
            ...
            if underlying_model.prefix_tokens is not None and "prefix_tokens" in best_ckpt:
                underlying_model.prefix_tokens.data.copy_(best_ckpt["prefix_tokens"].to(device))
