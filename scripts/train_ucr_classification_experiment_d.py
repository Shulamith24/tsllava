...
"""
实验 D: 小 Transformer 聚合头（验证迁移增益）

验证预训练 LLM 是否必要，还是小的从头训练 Transformer 就足够

核心改动：
- 使用 TSClassifierSmallTransformer（小 Transformer 替代 LLM）
- 输入序列：[TS tokens] + [ANS]
- 从头训练所有参数（无预训练）
- 参数量对齐 LoRA（约 5-15M）

使用方法：
    # medium 配置（推荐，3层 768维）
    python scripts/train_ucr_classification_experiment_d.py \
        --dataset Adiac \
...
def parse_args():
    parser = argparse.ArgumentParser(description="实验 D: 小 Transformer 聚合头")
    ...
    # 模型相关：聚合器配置
    parser.add_argument("--aggregator_config", type=str, default="medium", 
                       choices=["small", "medium", "large"], 
                       help="聚合器预设配置")
    parser.add_argument("--num_layers", type=int, default=None, help="Transformer层数（覆盖预设）")
    parser.add_argument("--hidden_size", type=int, default=None, help="Hidden维度（覆盖预设）")
    parser.add_argument("--num_heads", type=int, default=None, help="Attention heads（覆盖预设）")
    parser.add_argument("--ffn_dim", type=int, default=None, help="FFN维度（覆盖预设）")
    ...
    # 训练相关（从头训练的最佳实践）
    parser.add_argument("--epochs", type=int, default=50, help="训练轮数（从头训练需要更多）")
    parser.add_argument("--batch_size", type=int, default=16, help="批次大小")
    parser.add_argument("--lr", type=float, default=3e-4, help="基础学习率")
    parser.add_argument("--lr_encoder", type=float, default=None, help="编码器学习率（可选）")
    parser.add_argument("--weight_decay", type=float, default=0.01, help="权重衰减")
    parser.add_argument("--grad_clip", type=float, default=1.0, help="梯度裁剪")
    parser.add_argument("--warmup_ratio", type=float, default=0.1, help="预热比例（从头训练建议0.1）")
    parser.add_argument("--scheduler", type=str, default="cosine", 
                       choices=["cosine", "linear"], help="学习率调度器")
    
    # 正则化
    parser.add_argument("--label_smoothing", type=float, default=0.0, help="标签平滑（0.0-0.2）")
    
    # 保存相关
    parser.add_argument("--save_dir", type=str, default="results/experiment_d", help="结果保存目录")
    ...
    parser.add_argument("--early_stop", type=int, default=15, help="早停耐心值（从头训练建议更大）")
...
def save_checkpoint(
    ...
    checkpoint = {
        ...
        "aggregator_state": underlying_model.aggregator.state_dict(),
        ...
        "aggregator_config": {
            "num_layers": underlying_model.aggregator.num_layers,
            "hidden_size": underlying_model.hidden_size,
            "num_heads": underlying_model.aggregator.num_heads,
        },
        "args": vars(args),
    }
...
def main():
    ...
    if rank == 0:
        print("=" * 60)
        print("实验 D: 小 Transformer 聚合头")
        print("=" * 60)
        print(f"时间: {datetime.datetime.now()}")
        print(f"数据集: {args.dataset}")
        print(f"编码器: {args.encoder_type}")
        print(f"聚合器配置: {args.aggregator_config}")
    ...
    # 创建保存目录
    config_str = f"{args.aggregator_config}"
    if args.num_layers is not None:
        config_str += f"_L{args.num_layers}"
    if args.hidden_size is not None:
        config_str += f"_H{args.hidden_size}"
    ...
    model = TSClassifierSmallTransformer(
        ...
        aggregator_config=args.aggregator_config,
    ...
    # 使用统一学习率或分组学习率
    if args.lr_encoder is not None and not args.freeze_encoder:
        param_groups = [
            {"params": underlying_model.encoder.parameters(), "lr": args.lr_encoder},
            {"params": underlying_model.projector.parameters(), "lr": args.lr},
            {"params": underlying_model.aggregator.parameters(), "lr": args.lr},
            {"params": [underlying_model.ans_token], "lr": args.lr},
            {"params": underlying_model.classifier_head.parameters(), "lr": args.lr},
        ]
    else:
        # 统一学习率（推荐用于从头训练）
        param_groups = []
        if not args.freeze_encoder:
            param_groups.append({"params": underlying_model.encoder.parameters()})
        param_groups.extend([
            {"params": underlying_model.projector.parameters()},
            {"params": underlying_model.aggregator.parameters()},
            {"params": [underlying_model.ans_token]},
            {"params": underlying_model.classifier_head.parameters()},
        ])
        param_groups = [{"params": sum([list(g["params"]) for g in param_groups], []), "lr": args.lr}]
    ...
    try:
        ...
        if rank == 0:
            ...
            underlying_model.aggregator.load_state_dict(best_ckpt["aggregator_state"])
            underlying_model.classifier_head.load_state_dict(best_ckpt["classifier_head_state"])
            underlying_model.ans_token.data.copy_(best_ckpt["ans_token"].to(device))
            ...
            final_results = {
                ...
                "num_classes": num_classes,
                "aggregator_config": args.aggregator_config,
                "total_params": underlying_model.count_parameters(),
