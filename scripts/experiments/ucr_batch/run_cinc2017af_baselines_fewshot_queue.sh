...
: <<'USAGE'
Queue few-shot baseline experiments on CinC2017AF.
...
Example:
  ...
  bash scripts/experiments/ucr_batch/run_cinc2017af_baselines_fewshot_queue.sh \
    --cleanup_checkpoints
...
LAUNCHER_NAME="${LAUNCHER_NAME:-cinc2017af_baselines_fewshot}"
if [[ -z "${JOB_NAME_TEMPLATE:-}" ]]; then
  JOB_NAME_TEMPLATE="{experiment}_cinc2017af_fewshot"
fi
