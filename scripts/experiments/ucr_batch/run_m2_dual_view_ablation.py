...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--job_prefix", default=None, help="Optional prefix for generated batch job names.")
...
def main(argv: list[str] | None = None) -> int:
    ...
    reference_results = reference_job_dir / "results.txt"
    comparison_results = comparison_job_dir / "results.txt"
    candidate_datasets = shared_complete_datasets(
        [reference_results, comparison_results],
        dataset_source=args.data_path,
        shots=shots,
    )
    if not candidate_datasets:
        raise ValueError("No shared complete datasets found for the dual-view ablation candidate pool")
    ...
    if args.subset_manifest:
        ...
        if missing:
            raise ValueError(
                "subset_manifest contains datasets outside the current dual-view candidate pool: "
                + ",".join(missing)
            )
    else:
        ...
        subset_manifest = build_subset_manifest(
            candidate_pool_name=f"dual_view_shared_{len(candidate_datasets)}",
            dataset_source=args.data_path,
            shots=shots,
            candidate_datasets=candidate_datasets,
    ...
    report_name = args.report_name or f"m2_dual_view_ablation_{subset_id}"
    ...
    job_prefix = args.job_prefix or f"m2_dual_view_{subset_id}"
    ts_only_job_name = f"{job_prefix}_ts_only"
    vision_only_job_name = f"{job_prefix}_vision_only"

    base_batch_config_path = reference_job_dir / "batch_config.json"
    ts_only_forward_args = build_forward_args_from_reference(
        reference_batch_config_path=base_batch_config_path,
        local_checkpoint=args.local_checkpoint,
        shots=shots,
        num_runs=args.num_runs,
        runtime_branch_mode="ts_only",
    )
    vision_only_forward_args = build_forward_args_from_reference(
        reference_batch_config_path=base_batch_config_path,
        local_checkpoint=args.local_checkpoint,
        shots=shots,
        num_runs=args.num_runs,
        runtime_branch_mode="vision_only",
    )

    ts_only_job_dir = _run_batch_job(
        job_name=ts_only_job_name,
        data_path=str(args.data_path),
        datasets=selected_datasets,
        forward_args=ts_only_forward_args,
        gpu_ids=args.gpu_ids,
        dry_run=args.dry_run,
    )
    vision_only_job_dir = _run_batch_job(
        job_name=vision_only_job_name,
        data_path=str(args.data_path),
        datasets=selected_datasets,
        forward_args=vision_only_forward_args,
        gpu_ids=args.gpu_ids,
        dry_run=args.dry_run,
    )
    ...
    report_config = build_ablation_report_config(
        ...
        items=[
            ...
            },
            {
                "key": "ts_only",
                "label": "TS-only",
                "job_dir": str(ts_only_job_dir),
            },
            {
                "key": "vision_only",
                "label": "Vision-only",
                "job_dir": str(vision_only_job_dir),
            },
        ],
    )
    ...
    request_payload = build_run_request_payload(
        ...
        extra={
            ...
            "ts_only_job_name": ts_only_job_name,
            "vision_only_job_name": vision_only_job_name,
    ...
    if args.dry_run:
        print(f"Prepared dual-view ablation config at {report_config_path}")
