...
验证性能提升是否主要来自 LLM 作为深序列聚合器，而非：
- token 生成/大词表 softmax 的训练形式
- label tokenization 差异
- 文本 prompt 的语义

核心改动：
- 使用 OpenTSLMClassifier（基于 [ANS] token 的分类）
- 输入序列：[PrePrompt] + [TS tokens] + [PostPrompt] + [ANS]
- 训练目标：K 类分类（交叉熵损失）
- 使用 Llama 原始预训练权重（不加载 opentslm 模型）

使用方法：
    python scripts/train_ucr_classification_experiment_a.py \
        --dataset ECG5000 \
...
def parse_args():
    parser = argparse.ArgumentParser(description="实验 A: LLM 作为深序列聚合器验证")
    ...
    # 保存相关
    parser.add_argument("--save_dir", type=str, default="results/experiment_a", help="结果保存目录")
