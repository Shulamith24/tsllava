...
\begin{table*}[t]
\centering
\caption{Computational efficiency and fine-tuning cost under the 10-shot UCR protocol. Total parameters count the full deployed model, while updated parameters count only the parameters fine-tuned on the downstream support set. Adapter size estimates the per-dataset trainable-state footprint. Runtime and memory are measured on representative UCR datasets using a single RTX 3090.}
